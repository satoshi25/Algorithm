...
price_1 = [500, 300, 200, 50, 30, 10]
price_2 = [512, 256, 128, 64, 32]
...
while i < 7 or j < 5:
    for _ in range(i):
        contest1.append(price_1[i-1])
    if j < 6:
        for _ in range(2 ** (j-1)):
            contest2.append(price_2[j-1])
    i += 1
    j += 1
...
for _ in range(t_cnt):
    ...
    a_price, b_price = [0, 0]
    if a <= 21 and a > 0:
        a_price = contest1[a-1] * 10000
    if b <= 31 and b > 0:
        b_price = contest2[b-1] * 10000
