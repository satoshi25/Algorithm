...
참외밭은 ㄱ-자 모양이거나 ㄱ-자를 90도, 
180도, 270도 회전한 모양(┏, ┗, ┛ 모양)의 육각형이다. 
...
orders = []
f, s, t, result = [[0, 0], [0, 0], [0, 0], 0]
for i in range(6):
    cur = int(input().split()[1])
    orders.append(cur)
    if f[0] <= cur:
        f[0] = cur
        f[1] = i
if orders[(f[1] + 1) % 6] > orders[(f[1] - 1) % 6]:
    s[0], s[1] = orders[(f[1] + 1) % 6], f[1] + 1
else:
    s[0], s[1] = orders[(f[1] - 1) % 6], f[1] - 1

big = f[0] * s[0]
if f[1] > s[1]:
    result = big - orders[(f[1] + 2) % 6] * orders[(f[1] + 3) % 6]
else:
    result = big - orders[(s[1] + 2) % 6] * orders[(s[1] + 3) % 6]
print(result * per_cnt)
