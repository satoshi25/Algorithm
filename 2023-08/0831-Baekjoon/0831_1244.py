...
3번 스위치를 중심으로 2번, 4번 스위치의 상태가 같고 
1번, 5번 스위치의 상태가 같으므로, <그림 3>과 같이 
1번부터 5번까지 스위치의 상태를 모두 바꾼다. 
만약 <그림 2>에서 여학생이 4를 받았다면, 3번, 5번 스위치의 상태가 서로 다르므로 
4번 스위치의 상태만 바꾼다.
...
for _ in range(student_cnt):
    ...
    if m_f == "1":
        ...
        while True:
            if l == r:
                switch[l] = on_off(switch[l])
            else:
                if l < 0 or r >= switch_len:
                    break
                if switch[l] == switch[r]:
                    switch[l] = on_off(switch[l])
                    switch[r] = on_off(switch[r])
                else:
                    break
            r += 1
            l -= 1
