...
let [fileCnt, ...files] = require('fs')
  .readFileSync(path)
  .toString()
  .trim()
  .split('\n');

files = files.map((el) => el.split('.')[1]);
let extension = {};

for (let i = 0; i < +fileCnt; i++) {
  if (!extension[files[i]]) {
    extension[files[i]] = 1;
  } else {
    extension[files[i]]++;
  }
...
let sortArr = Object.keys(extension).sort((a, b) => (a > b ? 1 : -1));

let result = sortArr.reduce((acc, cur) => {
  acc.push(`${cur} ${extension[cur]}`);
  return acc;
}, []);
