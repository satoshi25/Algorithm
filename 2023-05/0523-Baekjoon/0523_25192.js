...
두 번째 줄부터 N 개의 줄에 걸쳐 
새로운 사람의 입장을 나타내는 ENTER, 혹은 채팅을 입력한 유저의 닉네임이 문자열로 주어진다. 
(문자열길이 1 <= 문자열 길이 <= 20)
...
for (let i = 0; i < chatLogs.length; i++) {
  let imojiSet = new Set(chatLogs[i]);
  imojiSet.delete('');
  result += imojiSet.size;
}

console.log(result);

// ENTER는 새로운 유저의 입장을 의미한다.
// ENTER 이후에 유저의 첫번째 채팅은 곰곰티콘이다.
// 새로운 유저 입장이 있고나서 다른 유저의 입장까지 여러 채팅을 하여도 곰곰티콘은 한번만 사용한다.

// ENTER를 기준으로 split 한다.
// 나눠진 배열을 순회하면서 그 안에 있는 요소의 중복을 없애기 위해 Set에 배열 안의 요소를 넣는다.
// ENTER 뒤에 채팅이 없는 경우가 있기에 빈 문자열을 제거한다.
// Set의 크기들을 더하여 결과를 출력한다.
