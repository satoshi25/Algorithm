...
let [logsCnt, ...logs] = require('fs')
  .readFileSync(path)
  .toString()
  .trim()
  .split('\n');

let curWorkers = new Set();

for (let i = 0; i < logsCnt; i++) {
  if (logs[i].slice(-5) === 'enter') {
    curWorkers.add(logs[i].slice(0, -6));
  } else {
    curWorkers.delete(logs[i].slice(0, -6));
  }
}

let workerList = [];

curWorkers.forEach((el) => workerList.push(el));
workerList.sort((a, b) => (a < b ? 1 : -1));

console.log(workerList.join('\n'));
