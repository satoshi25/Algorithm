...
i + 1번째 줄에는 i번째로 만난 사람들의 이름 Ai와 Bi가 공백을 사이에 두고 주어진다. 
...
let [logCnt, ...logs] = require('fs')
  .readFileSync(path)
  .toString()
  .trim()
  .split('\n');

let danceList = new Set(['ChongChong']);
for (let i = 0; i < logCnt; i++) {
  logs[i].split(' ').map((el, idx, arr) => {
    if (danceList.has(el)) {
      danceList.add(arr[+!idx]);
    }
  });
}

console.log(danceList.size);
