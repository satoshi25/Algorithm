...
let [treeCnt, ...treeSpots] = require('fs')
  .readFileSync(path)
  .toString()
  .trim()
  .split('\n');

let [treeInter] = treeSpots.reduce(
  (acc, cur) => {
    if (acc[1] !== 0) {
      acc[0].push(+cur - acc[1]);
      acc[1] = +cur;
    } else {
      acc[1] = +cur;
    }
...
let result = (treeSpots[+treeCnt - 1] - treeSpots[0]) / gcd - +treeCnt + 1;
console.log(result);
