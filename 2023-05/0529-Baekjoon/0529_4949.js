...
입력
각 문자열은 마지막 글자를 제외하고 
영문 알파벳, 공백, 소괄호("( )"), 대괄호("[ ]")로 이루어져 있으며, 온점(".")으로 끝나고, 
길이는 100글자보다 작거나 같다.
...
let sentences = require('fs').readFileSync(path).toString().trim().split('\n');
...
function vps(str) {
  ...
  if (strArr.length === 1 && strArr[0] === '.') {
    ...
    for (let i = 0; i < strArr.length; i++) {
      ...
      if (lBracket.test(strArr[i])) {
        stack.push(strArr[i]);
      } else {
        if (rBracket.test(strArr[i])) {
