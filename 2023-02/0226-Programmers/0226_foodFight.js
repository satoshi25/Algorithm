...
function solution(food) {
  var answer = '';
  let playerFood = food.reduce((total, cur, idx) => {
    if (idx === 0) {
      return total;
    } else {
      if (cur % 2 === 0) {
        total = total + String(idx).repeat(cur / 2);
      } else {
        total = total + String(idx).repeat((cur - 1) / 2);
      }
    }
    return total;
  }, '');

  answer = playerFood + '0' + playerFood.split('').reverse().join('');
