...
let str = require('fs').readFileSync(path).toString().trim().split('');

let [[zero, zCnt], [one, oCnt], i, len, pre] = [
  ['0', 0],
  ['1', 0],
  0,
  str.length,
...
while (i < len) {
  if (zero === str[i] && pre !== zero) {
    zCnt++;
  } else if (one === str[i] && pre !== one) {
    oCnt++;
  }
  pre = str[i];
  i++;
...
console.log(zCnt >= oCnt ? oCnt : zCnt);
