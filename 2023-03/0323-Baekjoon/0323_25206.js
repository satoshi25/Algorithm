...
인하대학교 컴퓨터공학과의 등급에 따른 과목평점은 아래와 같다.
A+	4.5
A0	4.0
B+	3.5
B0	3.0
...
rl.on('line', (line) => {
  ...
  let [rating, scoreSum] = input.reduce(
    (acc, cur) => {
      let subject = cur.split(' ');
      if (subject[2] !== 'P') {
        acc[0] += +subject[1] * scoreBoard[subject[2]];
        acc[1] += +subject[1];
      }
      return acc;
    },
  ...
  console.log((rating / scoreSum).toFixed(6));
  process.exit();
});
