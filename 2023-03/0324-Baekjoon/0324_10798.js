...
주어지는 글자는 영어 대문자 ‘A’부터 ‘Z’, 영어 소문자 ‘a’부터 ‘z’, 
숫자 ‘0’부터 ‘9’ 중 하나이다. 각 줄의 시작과 마지막에 빈칸은 없다.
...
rl.on('line', (line) => {
  ...
  if (line.trim().length > rows) {
    rows = line.trim().length;
  }
  columns++;
}).on('close', () => {
  let word = /[a-zA-Z0-9]/;
  ...
  for (let i = 0; i < rows; i++) {
    for (let j = 0; j < columns; j++) {
      if (word.test(input[j][i]) && input[j][i] !== undefined) {
        result += input[j][i];
      }
    }
  }
