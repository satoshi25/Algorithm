...
rl.on('line', (line) => {
  ...
  let [baskets, shot] = input[0].split(' ').map(Number);
  chance = input.slice(1);
  current = new Array(baskets + 1).fill(0);
  for (let i = 0; i < shot; i++) {
    let [aBasket, bBasket, boll] = chance[i].split(' ').map(Number);
    for (let j = aBasket; j <= bBasket; j++) {
      current[j] = boll;
    }
  }
  console.log(current.slice(1).join(' '));
});
