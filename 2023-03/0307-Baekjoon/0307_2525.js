...
let [curTime, cookTime] = require('fs')
  .readFileSync('/dev/stdin')
  .toString()
  .split('\n');
cookTime = Number(cookTime);
let [curHour, curMin] = curTime.split(' ').map((el) => Number(el));
let cookHour = Math.floor(cookTime / 60);
let cookMin = cookTime % 60;
let resultMin = (curMin + cookMin) % 60;
let extraHour = Math.floor((curMin + cookMin) / 60);
let resultHour = (curHour + cookHour + extraHour) % 24;
