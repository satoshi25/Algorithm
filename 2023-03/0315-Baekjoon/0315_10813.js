...
rl.on('line', (line) => {
  ...
  let [totalBaskets, shot] = input[0].split(' ').map(Number);
  let baskets = new Array(totalBaskets + 1).fill(0).map((el, idx) => idx);
  for (let i = 1; i <= shot; i++) {
    let [aBasket, bBasket] = input[i].split(' ');
    let temp = baskets[bBasket];
    baskets[bBasket] = baskets[aBasket];
    baskets[aBasket] = temp;
  }
